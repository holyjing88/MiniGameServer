...
func (s *Store) EnsureSchema(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx, deployments.SchemaSQL)
	return err
}
...
func (s *Store) GetAccount(ctx context.Context, appID, channel, openID string) (domain.PlayerRegister, bool, error) {
	...
	err := s.db.QueryRowContext(ctx, `
SELECT app_id, channel, player_id, IFNULL(open_id,''), IFNULL(username,''), IFNULL(click_id,''),
       platform_kind, registered_at_ms, IFNULL(extra_json,'')
FROM player_register WHERE app_id=? AND player_id=?`, appID, accountID,
	).Scan(&old.AppID, &old.Channel, &old.PlayerID, &old.OpenID, &old.Username, &old.ClickID,
...
INSERT INTO player_register
  (app_id, channel, player_id, open_id, username, click_id, platform_kind, registered_at_ms, extra_json)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		r.AppID, r.Channel, r.PlayerID, r.OpenID, r.Username, r.ClickID,
